...
resource "aws_ecs_task_definition" "definicion_tarea_veterinaria" {
...
resource "aws_ecs_service" "servicio_veterinaria" {
    name            = var.nombre_servicio_ecs
    cluster         = aws_ecs_cluster.cluster_veterinaria_servicios.id
    task_definition = aws_ecs_task_definition.definicion_tarea_veterinaria.arn
    desired_count   = 1
    launch_type     = "FARGATE"

    load_balancer {
        target_group_arn = aws_lb_target_group.tg_veterinaria.arn
    ...
    depends_on = [
        aws_ecs_task_definition.definicion_tarea_veterinaria,
...
resource "aws_appautoscaling_target" "obetivo_escalamiento_ecs" {
    ...
    resource_id        = "service/${aws_ecs_cluster.cluster_veterinaria_servicios.name}/${aws_ecs_service.servicio_veterinaria.name}"
...
resource "aws_lb_target_group" "tg_veterinaria" {
    name     = "tg-veterinaria"
    port     = 8080
    protocol = "HTTP"
    vpc_id   = data.aws_vpc.vpc_por_defecto.id
